...
{
   const int actions_num = atk_action_get_n_actions(action_obj);
   const char * const desc_set = "description test";
   const char *desc_get;

   for (int count = 0; actions_num > count; count++)
     {
        g_assert(NULL != atk_action_get_name(action_obj, count));
        g_assert(TRUE == atk_action_set_description(action_obj, count, desc_set));
        desc_get = atk_action_get_description(action_obj, count);
        g_assert(NULL != desc_get);
