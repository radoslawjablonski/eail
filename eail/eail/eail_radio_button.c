 ...
 */
G_DEFINE_TYPE_WITH_CODE(EailRadioButton, eail_radio_button, EAIL_TYPE_BUTTON,
...
_on_change(void *data, Evas_Object *obj, void *event_info)
{
   ...
   if (button->state == EINA_TRUE)
     button->state = EINA_FALSE;
   else
...
eail_radio_button_ref_state_set(AtkObject *obj)
{
   ...
   /*TODO not sure how to do this so let's compare values*/
