 ...
 * @param obj EailGengrid instance
 *
 * @returns reference to an stete set of accessible
 */
static AtkStateSet *
 ...
 *
 * @param parent EialGengrid instance
 * @param item EailGengrid child instance
...
/**
 * @brief Implemnetation of get_n_rows from AtkTable
 *
...
_get_item_pos(const Evas_Object *widget, gint *row, gint *column, gint index)
{
   ...
   elm_gengrid_item_pos_get(it, (unsigned int *)row, (unsigned int *) column);
...
eail_gengrid_selection_ref(AtkSelection *selection, gint i)
{
   ...
   child = eail_item_new(ATK_OBJECT(selection), ATK_ROLE_TABLE_CELL);
   atk_object_initialize(child, it);
 ...
 *
 * @param selection EailGengrid instnace
 *
